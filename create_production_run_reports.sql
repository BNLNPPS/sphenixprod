...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'run_report_status') THEN
        CREATE TYPE run_report_status AS ENUM (
            ...
            'partial',        -- some output events exist, but missing events remain after tolerance
            'missing'         -- no output events exist and missing events remain after tolerance
        );
    END IF;
END$$;
...
COMMENT ON COLUMN production_run_reports.incomplete_reasons IS 'Machine-readable concern labels such as missing_daqhosts, missing_segments, low_event_ratio, missing_segment_tail, missing_segment_gaps, short_segment_events, or error_codes.';
COMMENT ON COLUMN production_run_reports.status IS 'Run-level report status: complete, questionable, partial, or missing. Partial means some events are present but the run is still short; missing means no output events are present.';
