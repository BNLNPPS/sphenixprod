    ...
    def from_yaml(cls, yaml_data: Dict[str, Any], rule_name: str) -> "RuleConfig":
        ...
        for field in required_params_fields:
            if field not in params_data:
                raise ValueError(f"Missing required field '{field}' in params for rule '{rule_name}'.")
        ...
        for field in required_input_fields:
            if field not in input_data:
                raise ValueError(f"Missing required field '{field}' in input for rule '{rule_name}'.")

        # Extract and validate job
        job_data = rule_data.get("job", {})
        required_job_fields = ["arguments", "output_destination", "log", "accounting_group", "accounting_group_user",
                               "priority", "request_xferslots"]
        for field in required_job_fields:
            if field not in job_data:
                raise ValueError(f"Missing required field '{field}' in job for rule '{rule_name}'.")
        ...
        return cls(
            ...
            input=InputConfig(
                ...
                direct_path=input_data.get("direct_path")
            ),
            job=JobConfig(
